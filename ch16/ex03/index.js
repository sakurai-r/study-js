...
const ENCRYPT_FILE = "./encrypt64.json";
...
async function writeEncrypt64(data) {
  ...
  await fs.writeFile(ENCRYPT_FILE, JSON.stringify(data, null, 2));
}

async function readKey() {
  // ここを埋める（return Promise<鍵>）
  return Buffer.from(
    JSON.parse(await fs.readFile(KEY_FILE, "utf8")).key,
    "base64"
  );
}
...
async function readEncrypt64() {
  ...
  return JSON.parse(await fs.readFile(ENCRYPT_FILE, "utf-8"));
}
